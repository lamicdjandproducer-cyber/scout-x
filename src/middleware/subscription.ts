...
interface User { id: string; phone: string; name: string | null; stripe_customer_id: string | null; }
interface Subscription { id: string; user_id: string; status: string; current_period_end: Date | null; }
...
export async function getOrCreateUser(phone: string, name?: string): Promise<User> {
  ...
  if (!user) {
    user = await queryOne<User>(`INSERT INTO users (phone, name) VALUES ($1, $2) RETURNING *`, [phone, name || null]);
  }
  return user!;
}
...
export async function sendSubscriptionGate(phone: string): Promise<void> {
  const message = `Ola! Sou o *Scout X*, seu assistente de apostas esportivas com IA.

Para acessar as analises, voce precisa de uma assinatura ativa.

*Plano Scout X* - R$47/mes
Analises ilimitadas via WhatsApp
Cobertura de 7+ esportes
Value bets diarios
Suporte 24/7 via IA

*Assine agora:*
https://buy.stripe.com/scoutx

Apos o pagamento, envie qualquer mensagem para comecar!`;
...
export async function checkSubscriptionAndNotify(phone: string, userName?: string): Promise<{ user: User; hasAccess: boolean }> {
  ...
  if (!hasAccess) { await sendSubscriptionGate(phone); }
